/**
 * gps.js — GPS position tracking and path recording
 *
 * Handles GPS position watching, interval-based path logging, and IndexedDB persistence.
 * Records user's path while hiking, calculates distance, and allows exporting/clearing
 * recorded tracks. Integrates with the elevation graph and distance display.
 *
 * Exports:
 * - startPositionWatch, stopPositionWatch: GPS lifecycle
 * - startGPSLogger: Start logging coordinates at intervals
 * - loadSavedTrip, exportPathAsGeoJSON, clearPathData: Path management
 * - getPathSizeMB: Calculate IndexedDB storage usage
 */
...
export function startPositionWatch() {
    ...
    state.gpsWatchId = navigator.geolocation.watchPosition(
        (pos) => {
            ...
            document.getElementById('gps-status').innerText = `🛰️ ${state.latestFix.acc.toFixed(0)}m`;
            document.getElementById('elev-val').innerText = `${altFeet} ft`;
        },
        () => { document.getElementById('gps-status').innerText = "🛰️ ..."; },
        { enableHighAccuracy: true, maximumAge: 0 }
    );
}

export function stopPositionWatch() {
    if (state.gpsWatchId !== null) {
        navigator.geolocation.clearWatch(state.gpsWatchId);
        state.gpsWatchId = null;
    }
    state.latestFix = null;
    document.getElementById('gps-status').innerText = "🛰️ Off";
}
