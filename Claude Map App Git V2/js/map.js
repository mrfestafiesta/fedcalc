/**
 * map.js — Leaflet map instance and layer groups
 *
 * Instantiates the Leaflet map and exports all layer groups (markers, routes, trails, etc.)
 * that are used across the application. This centralized approach prevents circular dependencies
 * and provides a single source for the map instance.
 *
 * Exports:
 * - map: Leaflet map instance (no zoom control, default view centered on USA)
 * - Layer groups: osmLayer, voyagerLayer, cartoLayer, satelliteLayer, federalLandsLayer,
 *   campgroundLayer, userMarker, pathLine, routeLayer, dotLayer, dynamicPoiLayer, trailPathLayer
 * - updateZoomStatus: Called on every zoomend event to update UI
 */

export const map = L.map('map', { zoomControl: false }).setView([39.8283, -98.5795], 4);
...
export const campgroundLayer = L.layerGroup();
...
function updateZoomStatus() {
    ...
    const digital = z > MAX_NATIVE_ZOOM;
    zoomStatus.textContent = digital ? `🔍 ${z} / ${MAX_NATIVE_ZOOM} ⚠️` : `🔍 ${z} / ${MAX_NATIVE_ZOOM}`;
    zoomStatus.classList.toggle('digital', digital);
}
