/**
 * storage.js — Cache management, service worker registration, and update detection
 *
 * Manages offline storage, cache sizing, service worker lifecycle, and the force update
 * system that ensures users always have the latest app code and park data.
 *
 * Exports:
 * - CACHES: Map of cache bucket names (app shell, data, tiles, weather)
 * - getCacheSizeMB, refreshStorageNumbers: Calculate displayed cache sizes
 * - requestWakeLock, registerServiceWorker: Device and lifecycle utilities
 * - forceUpdate: Check and update all app shell files and park data
 * - showStatusPill: Display transient status messages to user
 */

import { state } from './state.js';
import { getPathSizeMB } from './gps.js';

export const CACHES = { app: 'tracker-shell-v1', map: 'map-tiles', poi: 'tracker-data-v1', weather: 'weather-data-v1' };

/* === CACHE SIZING AND DISPLAY === */

export async function getCacheSizeMB(cacheName) {
    ...
    try {
        ...
        if (requests.length === 0) return "0.00";
        const sizes = await Promise.all(
            requests.map(req => cache.match(req).then(res => res ? res.blob().then(b => b.size) : 0))
        );
...
export async function refreshStorageNumbers() {
    ...
    const sizeWeather = document.getElementById('size-weather');
    sizeApp.innerText = "Calc..."; sizeMap.innerText = "Calc...";
    sizePoi.innerText = "Calc..."; sizePath.innerText = "Calc..."; sizeWeather.innerText = "Calc...";
    sizeApp.innerText = `${await getCacheSizeMB(CACHES.app)} MB`;
    sizeMap.innerText = `${await getCacheSizeMB(CACHES.map)} MB`;
    sizePoi.innerText = `${await getCacheSizeMB(CACHES.poi)} MB`;
    sizePath.innerText = `${await getPathSizeMB()} MB`;
    sizeWeather.innerText = `${await getCacheSizeMB(CACHES.weather)} MB`;
}
...
export async function requestWakeLock() {
    ...
    try {
        ...
        btnWake.textContent = 'Enabled';
        btnWake.style.background = '#28a745';
        state.wakeLockSentinel.addEventListener('release', () => {
            if (!state.isWakeLockRequested) {
                btnWake.textContent = 'Disabled';
                btnWake.style.background = '#d9534f';
            }
        });
    } catch (err) {
...
export async function forceUpdate() {
    ...
    try {
        ...
        const shellFiles = [
            ...
            './js/weather/weather.js', './js/weather/nws.js', './js/weather/open-meteo.js', './js/weather/utils.js',
            './js/popups.js', './js/routing.js', './js/ar.js',
