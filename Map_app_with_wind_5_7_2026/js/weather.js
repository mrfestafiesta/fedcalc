...
function formatWind(speed, direction) {
    ...
    return direction !== undefined ? `${spd} mph ${degreesToCardinal(direction)}` : `${spd} mph`;
}

export async function fetchAndSetPOIData(marker, lat, lng, name) {
    try {
        const res = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lng}&current_weather=true&temperature_unit=fahrenheit&wind_speed_unit=mph&timezone=auto`);
        const data = await res.json();
        const cw = data.current_weather;
        const currentTemp = cw ? Math.round(cw.temperature) : '--';
        const wind = cw ? formatWind(cw.windspeed, cw.winddirection) : '--';
...
export async function fetchForecast(lat, lng, mode, poiName) {
    ...
    try {
        let url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lng}&temperature_unit=fahrenheit&wind_speed_unit=mph&timezone=auto`;
        if (mode === 'weekly') {
            url += '&daily=weather_code,temperature_2m_max,temperature_2m_min,windspeed_10m_max,winddirection_10m_dominant';
        } else {
            url += '&hourly=temperature_2m,weather_code,windspeed_10m&forecast_days=2';
        }
        ...
        if (mode === 'weekly') {
            for (let i = 0; i < 7; i++) {
                ...
                const wind = formatWind(data.daily.windspeed_10m_max[i], data.daily.winddirection_10m_dominant[i]);
                html += `
                    <div style="padding: 8px 0; border-bottom: 1px solid #eee;">
                        ...
                        <div style="text-align: right; color: #666; font-size: 12px; margin-top: 2px;">💨 ${wind}</div>
                    </div>`;
            }
        } else {
            ...
            for (let i = 0; i < data.hourly.time.length; i++) {
                ...
                if (forecastTime >= currentHourObj && hoursShown < 24) {
                    if (hoursShown % 3 === 0) {
                        ...
                        const wind = formatWind(data.hourly.windspeed_10m[i]);
                        html += `
